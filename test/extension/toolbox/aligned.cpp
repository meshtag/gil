...
void pixel_fill_rgb(std::vector<std::vector<std::vector<int>>>& vec, gil::rgb8_image_t& img)
{
    for (std::ptrdiff_t view_row = 0; view_row < view(img).height(); ++view_row)
    {
        ...
        {
            view(img)(view_col, view_row) = gil::rgb8_pixel_t(vec[view_row][view_col][0],
...
int main()
{
    ...
    BOOST_TEST(gil::equal_pixels(obtained_center_middle_view, view(expected_center_middle_img)));
    BOOST_TEST(gil::equal_pixels(obtained_left_top_view, view(expected_left_top_img)));

    std::vector<int> vec_ocm, vec_ecm, vec_olt, vec_elt;
    view_to_vector(obtained_center_middle_view, vec_ocm);
    view_to_vector(gil::view(expected_center_middle_img), vec_ecm);
    view_to_vector(obtained_left_top_view, vec_olt);
    view_to_vector(gil::view(expected_left_top_img), vec_elt);

    BOOST_TEST_ALL_EQ(vec_ocm.begin(), vec_ocm.end(), vec_ecm.begin(), vec_ecm.end());
    BOOST_TEST_ALL_EQ(vec_olt.begin(), vec_olt.end(), vec_elt.begin(), vec_elt.end());
