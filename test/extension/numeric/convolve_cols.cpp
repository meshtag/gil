...
struct test_image_5x5_kernel_1x9_boundary_extend_padded
{
    ...
    void operator()(Image const&)
    {
        ...
        // First column of "img_out" and "img_expected" are intentionally made similar.
        auto img_out_it = gil::view(img_out).row_begin(0);
        auto img_expected_it = gil::view(img_expected).row_begin(0);
        for (std::ptrdiff_t y = 0; y < gil::view(img_out).height(); ++y)
            img_expected_it[y] = img_out_it[y];
